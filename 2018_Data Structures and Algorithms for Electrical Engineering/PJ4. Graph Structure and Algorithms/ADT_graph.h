...
class ADT_Node {
	public:
		...
		// New Method
		int set_path(string node_path);
		string get_path(void);
		int clear_path(void);
		int set_node_value(T node_value);
		set<string> get_cluster(void);
		int set_cluster(set<string> node_cluster);
		int initialize_cluster(void);
...
class ADT_Edge { 
	public:
		/* function */
		string get_edge_name(void) const;
		T get_edge_value(void) const;
...
class ADT_Nodelist {
	public:
		...
		//New method
		int clear_node_path(void);
		int set_all_node_value(T node_value);
...
class ADT_Graph {
	public:
		int vertices(void);
		int edges(void);
		// 각 insertVertex와 insertEdge에 대해서 중복 node_name은 불허해야할듯
		...
		// 각 erase 함수에 대해서 존재하지 않는 node_name에 대해서는 불허해야할듯 
