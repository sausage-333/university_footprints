...
int main(int argc, char* argv[])
{
	...
	// STEP 1: 단어들을 읽고, RBTree에 저장하는 과정
	if (argc != 3) {
	...
	for (line_it; line_it != line_list.end(); ++line_it) {
		...
		while (delimiter) {
			...
			rbt.insert(word);
			rbt.search(word)->value->push_back(line_number);
		}
	...
	//cout << "Running Time for inserting all words in RB Tree: " << difftime(end1, start1) << " s" << endl;
	...
	do {
		...
		if (command == "R") {
			...
			if (rbt.is_external(rbt.search(word1))) {
			...
			// 만약 같은 단어를 두고 대체하려 했다면, 에러 메시지 표시.
				...
				rbt.insert(word2);
				RBNode<list<int>>* word2_node = rbt.search(word2);
				*(word2_node->value) = *(rbt.search(word1)->value);
				rbt.remove(word1);
			...
			//cout << "It took " << difftime(end2, start2) << " s to replace word in RB Tree." << endl;
		}
		else if (command == "Q") {
			break;
		}
